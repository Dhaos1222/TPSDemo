...
public class WeaponController : MonoBehaviour
{
    ...
    public float m_CurrentAmmo { get; set; }
    ...
    void OnGUI()
    {
        // GUI.Label(new Rect(25, 25, 200, 20),  "Weapon Ammo : " + m_CurrentAmmo);
    }

    void UpdateAmmo()
    {
        if (m_LastTimeShot + ammoReloadDelay < Time.time && m_CurrentAmmo < maxAmmo && isReloading)
        {
    ...
    public bool ReloadAmmo(bool inputDown)
    {
        if(inputDown)
        {
    ...
    public bool SwitchFireMode(bool inputDown)
    {
        if(inputDown)
        {
            switch(shootType)
